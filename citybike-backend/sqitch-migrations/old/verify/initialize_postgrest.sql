...
BEGIN
    ASSERT EXISTS(
        ...
        WHERE
            schema_name =(utils.get_api_schema())),
    ...
    ASSERT EXISTS(
        ...
        WHERE
            grantee =(utils.get_auth_role())
            AND role_name =(utils.get_anon_role())),
    'Authenticator role should be able to  become anonymous role.';
    ASSERT(
        SELECT
            pg_catalog.HAS_SCHEMA_PRIVILEGE((utils.get_anon_role()),(utils.get_api_schema()), 'USAGE')),
    'Anonymous role should have usage on api.';
    ASSERT NOT(
        SELECT
            pg_catalog.HAS_SCHEMA_PRIVILEGE((utils.get_auth_role()),(utils.get_api_schema()), 'USAGE')),
